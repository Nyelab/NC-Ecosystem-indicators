...
df, dep_var, intercept, pdata, mod.dsig, y_units, x_units, title_name


VesselLength_ratio <- VesselLength %>% mutate(division= ifelse(vessellengthrange %in% c("0-18ft.", "19-38ft."), "small", "large")) %>% group_by(year, division) %>% mutate(total_count= sum(count)) %>% distinct(total_count, .keep_all=TRUE) %>% select(year, division, total_count) %>% spread(division, total_count, fill= 0) %>% mutate(ratio= small/large)
k_det(VesselLength_ratio)
gam_diagn(VesselLength_ratio$ratio, 7, VesselLength_ratio) #p-value was signficant at 6
mod_VesselLength_ratio <- gam(ratio ~ s(year, k= 6), data = VesselLength_ratio)
