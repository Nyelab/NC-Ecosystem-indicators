...
RecFishing <- read_csv("data/finalized/impacts and responses/e1981RecreationalEffortNCSD.csv")
...
RecCatch <- read_csv("/users/sallydowd/Desktop/b1981RecreationalTotalCatchNCSD.csv")
RecRelAlive <- read_csv("/users/sallydowd/Desktop/b1981RecRelAliveNCSD.csv")
...
RecLicenseCoast <- read_csv("data/finalized/s2007RecLicensesCoastalNCSD.csv")
RecLicenseNonCoast <- read_csv("data/finalized/s2007RecLicensesNonCoastalNCSD.csv")
RecLicenseInState <- read_csv("data/finalized/s2007RecLicensesInStateNCSD.csv")
RecLicenseOutofState <- read_csv("data/finalized/s2007RecLicensesOutofStateNCSD.csv")
RecLicenseTotal <- read_csv("data/finalized/s2007RecLicensesTotalNCSD.csv")
setwd("/Users/sallydowd/Documents/GitHub/NC-Ecosystem-indicators")
NTurbidity <- read_csv("data/finalized/p2008NPamlicoTurbiditySD.csv")
CTurbidity <- read_csv("data/finalized/p2008CPamlicoTurbiditySD.csv")
CBTurbidity <- read_csv("data/finalized/p2008CBPamlicoTurbiditySD.csv")
STurbidity <- read_csv("data/finalized/p2008SPamlicoTurbiditySD.csv")
setwd("/Users/sallydowd/Documents/GitHub/NC-Ecosystem-indicators")

setwd("/Users/sallydowd/Documents/GitHub/NC-Ecosystem-indicators")
VesselsNum <- read.csv("data/finalized/impacts and responses/s1992VesselRegistration.csv")
VesselsNum <-VesselsNum %>% dplyr::select(-X)
VesselsNum_cf <- read.csv("/users/sallydowd/Desktop/vessel_nums_cf.csv")
VesselsNum_caf <- read.csv("/users/sallydowd/Desktop/vessel_nums_caf.csv")

names(RecFishing) <- tolower(names(RecFishing))
names(CommLicense) <- tolower(names(CommLicense))
CommLicense <- filter(CommLicense, year >= 2000)
...
ShmpReg <- filter(ShmpReg, year < 2020)
names(BeachNouris) <- tolower(names(BeachNouris))
names(RecLicenseCoast) <- tolower(names(RecLicenseCoast))
names(RecLicenseNonCoast) <- tolower(names(RecLicenseNonCoast))
names(RecLicenseInState) <- tolower(names(RecLicenseInState))
names(RecLicenseOutofState) <- tolower(names(RecLicenseOutofState))
names(RecLicenseTotal) <- tolower(names(RecLicenseTotal))
names(VesselsNum) <- tolower(names(VesselsNum))
names(VesselsNum_cf) <- tolower(names(VesselsNum_cf))
names(VesselsNum_caf) <- tolower(names(VesselsNum_caf))
```
...
##Rec fishing 
gam_diagn(RecFishing$angler_trips, 7, RecFishing) #p value not significant 
mod_RecFishing <- gam(angler_trips ~ s(year, k= 7), data = RecFishing)
...
##Vessel length part 2 
VesselLength_edt <- VesselLength %>% mutate(division= ifelse(vessellengthrange %in% c("0-18ft.", "19-38ft."), 1, 2))

###Under 38 
VesselLength_under38 <- VesselLength_edt %>% filter(division ==1) %>% group_by(year) %>% mutate(total_count= sum(count)) %>% distinct(total_count, .keep_all=TRUE) %>% select(year, division, total_count)
k_det(VesselLength_under38)
gam_diagn(VesselLength_under38$total_count, 6, VesselLength_under38) 
mod_VesselLength_under38 <- gam(total_count ~ s(year, k= 6), data = VesselLength_under38)

###Over 38 
VesselLength_over38 <- VesselLength_edt %>% filter(division ==2) %>% group_by(year) %>% mutate(total_count= sum(count)) %>% distinct(total_count, .keep_all=TRUE) %>% select(year, division, total_count)
k_det(VesselLength_over38)
gam_diagn(VesselLength_over38$total_count, 6, VesselLength_over38) 
mod_VesselLength_over38 <- gam(total_count ~ s(year, k= 6), data = VesselLength_over38)

##Vessel length part 3
VesselLength_ratio <- VesselLength %>% mutate(division= ifelse(vessellengthrange %in% c("0-18ft.", "19-38ft."), "small", "large")) %>% group_by(year, division) %>% mutate(total_count= sum(count)) %>% distinct(total_count, .keep_all=TRUE) %>% select(year, division, total_count) %>% spread(division, total_count, fill= 0) %>% mutate(ratio= small/large) %>% filter(year >= 2000)
k_det(VesselLength_ratio)
gam_diagn(VesselLength_ratio$ratio, 7, VesselLength_ratio) #p-value was signficant at 6
mod_VesselLength_ratio <- gam(ratio ~ s(year, k= 6), data = VesselLength_ratio)

##Vessel length: large to small ratio
VesselLength_ratio2 <- VesselLength %>% mutate(division= ifelse(vessellengthrange %in% c("0-18ft.", "19-38ft."), "small", "large")) %>% group_by(year, division) %>% mutate(total_count= sum(count)) %>% distinct(total_count, .keep_all=TRUE) %>% select(year, division, total_count) %>% spread(division, total_count, fill= 0) %>% mutate(ratio= large/small) %>% filter(year >= 2000)
k_det(VesselLength_ratio2)
gam_diagn(VesselLength_ratio2$ratio, 6, VesselLength_ratio2) #p-value was significant at 4
mod_VesselLength_ratio2 <- gam(ratio ~ s(year, k= 6), data = VesselLength_ratio2)
...
k_det(BirdNests)
gam_diagn(BirdNests$sum_nests, 10, BirdNests) #assumptions aren't met but doesn't get better with k 
mod_BirdNests <- gam(sum_nests ~ s(year, k= 10), data = BirdNests)
...
##Rec Rel Alive
k_det(RecRelAlive)
gam_diagn(RecRelAlive$relalive, 8, RecRelAlive) #p not sig at k= 6, graphs don't get much better 
mod_RecRelAlive <- gam(relalive ~ s(year, k= 8), data = RecRelAlive)
...
##Rec licenses
k_det(RecLicenseCoast) #4

###RecLicenseCoast
gam_diagn(RecLicenseCoast$number, 4, RecLicenseCoast)
mod_RecLicenseCoast <- gam(number ~ s(year, k= 10), data = RecLicenseCoast)

###RecLicenseNonCoast
gam_diagn(RecLicenseNonCoast$number, 4, RecLicenseNonCoast)
mod_RecLicenseNonCoast <- gam(number ~ s(year, k= 4), data = RecLicenseNonCoast)

###RecLicenseInState
gam_diagn(RecLicenseInState$number, 4, RecLicenseInState)
mod_RecLicenseInState <- gam(number ~ s(year, k= 4), data = RecLicenseInState)

###RecLicenseOutofState
gam_diagn(RecLicenseOutofState$number, 4, RecLicenseOutofState)
mod_RecLicenseOutofState <- gam(number ~ s(year, k= 4), data = RecLicenseOutofState)

###RecLicenseTotal
gam_diagn(RecLicenseTotal$number, 4, RecLicenseTotal)
mod_RecLicenseTotal <- gam(number ~ s(year, k= 10), data = RecLicenseTotal)

##Turbidity

##NTurbidity
k_det(NTurbidity)
gam_diagn(NTurbidity$mean_turbidity, 4, NTurbidity)
mod_NTurbidity <- gam(mean_turbidity ~ s(year, k= 4), data = NTurbidity)

##CTurbidity
gam_diagn(CTurbidity$mean_turbidity, 4, CTurbidity)
mod_CTurbidity <- gam(mean_turbidity ~ s(year, k= 4), data = CTurbidity)

##CBTurbidity
gam_diagn(CBTurbidity$mean_turbidity, 4, CBTurbidity)
mod_CBTurbidity <- gam(mean_turbidity ~ s(year, k= 4), data = CBTurbidity)

##STurbidity
gam_diagn(STurbidity$mean_turbidity, 4, STurbidity)
mod_STurbidity <- gam(mean_turbidity ~ s(year, k= 4), data = STurbidity)

##VesselsNum
gam_diagn(VesselsNum$Num_reg, 10, VesselsNum)
mod_VesselsNum <- gam(Num_reg ~ s(year, k= 10), data = VesselsNum)

##VesselsNum_cf
gam_diagn(VesselsNum_cf$num_reg, 4, VesselsNum_cf)
mod_VesselsNum_cf <- gam(num_reg ~ s(year, k= 4), data = VesselsNum_cf)
```
...
linearMod_1997<- lm(angler_trips ~ year, data=dfpost1997_RecFishing)
...
ggsave("/users/sallydowd/Desktop/RecFishingGAMwithTrend.png", width = 6, height = 4, dpi = 300)
...
linearMod_1997 <- lm(licenses ~ year, dfpost1997_CommLicense_SCFL)
summary(linearMod_1997)
...
#Vessel length 2 
##Under 38
p2mod_VesselLength_under38 <- GAM_predict_p2mod(VesselLength_under38, mod_VesselLength_under38)
intercept_VesselLength_under38 = as.numeric(attr(p2mod_VesselLength_under38,"constant")) 
pdata_VesselLength_under38 <- GAM_predict_pdata(VesselLength_under38, mod_VesselLength_under38)
mod.dsig_VesselLength_under38 <- GAM_deriv(pdata_VesselLength_under38, mod_VesselLength_under38)
linearMod <- lm(total_count ~ year, data= VesselLength_under38)
summary(linearMod)
dfpost1997_VesselLength_under38 <- filter(VesselLength_under38, year > 1997)
linearMod_1997 <- lm(total_count ~ year, data=dfpost1997_VesselLength_under38)
summary(linearMod_1997)
both_sig(VesselLength_under38, VesselLength_under38$total_count, intercept_VesselLength_under38, pdata_VesselLength_under38, mod.dsig_VesselLength_under38, dfpost1997_VesselLength_under38, dfpost1997_VesselLength_under38$total_count, "Number of Vessels", "Year", "Number of Vessels under 38 ft.") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("/users/sallydowd/Desktop/Figures/", "VesselLength_under38GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##Over 38
p2mod_VesselLength_over38 <- GAM_predict_p2mod(VesselLength_over38, mod_VesselLength_over38)
intercept_VesselLength_over38 = as.numeric(attr(p2mod_VesselLength_over38,"constant")) 
pdata_VesselLength_over38 <- GAM_predict_pdata(VesselLength_over38, mod_VesselLength_over38)
mod.dsig_VesselLength_over38 <- GAM_deriv(pdata_VesselLength_over38, mod_VesselLength_over38)
linearMod <- lm(total_count ~ year, data= VesselLength_over38)
summary(linearMod)
dfpost1997_VesselLength_over38 <- filter(VesselLength_over38, year > 1997)
linearMod_1997 <- lm(total_count ~ year, data=dfpost1997_VesselLength_over38)
summary(linearMod_1997)
both_sig(VesselLength_over38, VesselLength_over38$total_count, intercept_VesselLength_over38, pdata_VesselLength_over38, mod.dsig_VesselLength_over38, dfpost1997_VesselLength_over38, dfpost1997_VesselLength_over38$total_count, "Number of Vessels", "Year", "Number of Vessels over 38 ft.") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("/users/sallydowd/Desktop/Figures/", "VesselLength_over38GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##Vessel length 3: ratio of small to large
p2mod_VesselLength_ratio <- GAM_predict_p2mod(VesselLength_ratio, mod_VesselLength_ratio)
intercept_VesselLength_ratio = as.numeric(attr(p2mod_VesselLength_ratio,"constant")) 
pdata_VesselLength_ratio <- GAM_predict_pdata(VesselLength_ratio, mod_VesselLength_ratio)
mod.dsig_VesselLength_ratio <- GAM_deriv(pdata_VesselLength_ratio, mod_VesselLength_ratio)
linearMod <- lm(ratio ~ year, data= VesselLength_ratio)
summary(linearMod)
dfpost1997_VesselLength_ratio <- filter(VesselLength_ratio, year > 1997)
linearMod_1997 <- lm(ratio ~ year, data=dfpost1997_VesselLength_ratio)
summary(linearMod_1997)
both_nonsig(VesselLength_ratio, VesselLength_ratio$ratio, intercept_VesselLength_ratio, pdata_VesselLength_ratio, mod.dsig_VesselLength_ratio, "Ratio of Vessels", "Year", "Ratio of small to large vessels") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("/users/sallydowd/Desktop/Indicators/Figures/", "VesselLength_ratioGAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##Vessel length ratio: large to small
p2mod_VesselLength_ratio2 <- GAM_predict_p2mod(VesselLength_ratio2, mod_VesselLength_ratio2)
intercept_VesselLength_ratio2 = as.numeric(attr(p2mod_VesselLength_ratio2,"constant")) 
pdata_VesselLength_ratio2 <- GAM_predict_pdata(VesselLength_ratio2, mod_VesselLength_ratio2)
mod.dsig_VesselLength_ratio2 <- GAM_deriv(pdata_VesselLength_ratio2, mod_VesselLength_ratio2)
linearMod <- lm(ratio ~ year, data= VesselLength_ratio2)
summary(linearMod)
dfpost1997_VesselLength_ratio2 <- filter(VesselLength_ratio2, year > 1997)
linearMod_1997 <- lm(ratio ~ year, data=dfpost1997_VesselLength_ratio2)
summary(linearMod_1997)
both_nonsig(VesselLength_ratio2, VesselLength_ratio2$ratio, intercept_VesselLength_ratio2, pdata_VesselLength_ratio2, mod.dsig_VesselLength_ratio2, "Ratio of Vessels", "Year", "Ratio of large to small vessels") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("/users/sallydowd/Desktop/Figures/", "VesselLength_ratio2GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')
...
ggsave("/users/sallydowd/Desktop/RecCatchAMwithTrend.png", width = 6, height = 4, dpi = 300)
...
ggsave("/users/sallydowd/Desktop/RecRelAliveAMwithTrend.png", width = 6, height = 4, dpi = 300)
ggsave(paste0("/users/sallydowd/Desktop", "RecRelAliveAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')
...
##Coastal recreational licenses
p2mod_RecLicenseCoast <- GAM_predict_p2mod(RecLicenseCoast, mod_RecLicenseCoast)
intercept_RecLicenseCoast = as.numeric(attr(p2mod_RecLicenseCoast,"constant")) 
pdata_RecLicenseCoast <- GAM_predict_pdata(RecLicenseCoast, mod_RecLicenseCoast)
mod.dsig_RecLicenseCoast <- GAM_deriv(pdata_RecLicenseCoast, mod_RecLicenseCoast)
linearMod <- lm(number ~ year, data= RecLicenseCoast)
summary(linearMod)
dfpost1997_RecLicenseCoast <- filter(RecLicenseCoast, year > 1997)
linearMod_1997 <- lm(number ~ year, data=dfpost1997_RecLicenseCoast)
summary(linearMod_1997)
both_nonsig(RecLicenseCoast, RecLicenseCoast$number, intercept_RecLicenseCoast, pdata_RecLicenseCoast, mod.dsig_RecLicenseCoast, "Number", "Year", "Number of CRFL Licenses\n Purchased in Coastal Counties") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "RecLicenseCoast.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##Non-coastal recreational licenses
p2mod_RecLicenseNonCoast <- GAM_predict_p2mod(RecLicenseNonCoast, mod_RecLicenseNonCoast)
intercept_RecLicenseNonCoast = as.numeric(attr(p2mod_RecLicenseNonCoast,"constant")) 
pdata_RecLicenseNonCoast <- GAM_predict_pdata(RecLicenseNonCoast, mod_RecLicenseNonCoast)
mod.dsig_RecLicenseNonCoast <- GAM_deriv(pdata_RecLicenseNonCoast, mod_RecLicenseNonCoast)
linearMod <- lm(number ~ year, data= RecLicenseNonCoast)
summary(linearMod)
dfpost1997_RecLicenseNonCoast <- filter(RecLicenseNonCoast, year > 1997)
linearMod_1997 <- lm(number ~ year, data=dfpost1997_RecLicenseNonCoast)
summary(linearMod_1997)
both_sig(RecLicenseNonCoast, RecLicenseNonCoast$number, intercept_RecLicenseNonCoast, pdata_RecLicenseNonCoast, mod.dsig_RecLicenseNonCoast, dfpost1997_RecLicenseNonCoast, dfpost1997_RecLicenseNonCoast$number, "Number", "Year", "Number of CRFL Licenses\n Purchased in Non-coastal Counties") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "RecLicenseNonCoast.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##In-State recreational licenses
p2mod_RecLicenseInState <- GAM_predict_p2mod(RecLicenseInState, mod_RecLicenseInState)
intercept_RecLicenseInState = as.numeric(attr(p2mod_RecLicenseInState,"constant")) 
pdata_RecLicenseInState <- GAM_predict_pdata(RecLicenseInState, mod_RecLicenseInState)
mod.dsig_RecLicenseInState <- GAM_deriv(pdata_RecLicenseInState, mod_RecLicenseInState)
linearMod <- lm(number ~ year, data= RecLicenseInState)
summary(linearMod)
dfpost1997_RecLicenseInState <- filter(RecLicenseInState, year > 1997)
linearMod_1997 <- lm(number ~ year, data=dfpost1997_RecLicenseInState)
summary(linearMod_1997)
both_nonsig(RecLicenseInState, RecLicenseInState$number, intercept_RecLicenseInState, pdata_RecLicenseInState, mod.dsig_RecLicenseInState, "Number", "Year", "Number of CRFL Licenses\n Purchased in NC Counties") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "RecLicenseInState.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##Out-of-State recreational licenses
p2mod_RecLicenseOutofState <- GAM_predict_p2mod(RecLicenseOutofState, mod_RecLicenseOutofState)
intercept_RecLicenseOutofState = as.numeric(attr(p2mod_RecLicenseOutofState,"constant")) 
pdata_RecLicenseOutofState <- GAM_predict_pdata(RecLicenseOutofState, mod_RecLicenseOutofState)
mod.dsig_RecLicenseOutofState <- GAM_deriv(pdata_RecLicenseOutofState, mod_RecLicenseOutofState)
linearMod <- lm(number ~ year, data= RecLicenseOutofState)
summary(linearMod)
dfpost1997_RecLicenseOutofState <- filter(RecLicenseOutofState, year > 1997)
linearMod_1997 <- lm(number ~ year, data=dfpost1997_RecLicenseOutofState)
summary(linearMod_1997)
both_sig(RecLicenseOutofState, RecLicenseOutofState$number, intercept_RecLicenseOutofState, pdata_RecLicenseOutofState, mod.dsig_RecLicenseOutofState, dfpost1997_RecLicenseOutofState, dfpost1997_RecLicenseOutofState$number, "Number", "Year", "Number of CRFL Licenses\n Purchased outside of NC") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "RecLicenseOutofState.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##Total recreational licenses
p2mod_RecLicenseTotal <- GAM_predict_p2mod(RecLicenseTotal, mod_RecLicenseTotal)
intercept_RecLicenseTotal = as.numeric(attr(p2mod_RecLicenseTotal,"constant")) 
pdata_RecLicenseTotal <- GAM_predict_pdata(RecLicenseTotal, mod_RecLicenseTotal)
mod.dsig_RecLicenseTotal <- GAM_deriv(pdata_RecLicenseTotal, mod_RecLicenseTotal)
linearMod <- lm(number ~ year, data= RecLicenseTotal)
summary(linearMod)
dfpost1997_RecLicenseTotal <- filter(RecLicenseTotal, year > 1997)
linearMod_1997 <- lm(number ~ year, data=dfpost1997_RecLicenseTotal)
summary(linearMod_1997)
both_sig(RecLicenseTotal, RecLicenseTotal$number, intercept_RecLicenseTotal, pdata_RecLicenseTotal, mod.dsig_RecLicenseTotal, dfpost1997_RecLicenseTotal, dfpost1997_RecLicenseTotal$number, "Number", "Year", "Number of CRFL Licenses\n Purchased by Year") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "RecLicenseTotal.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

#Turbidity
##NTurbidity
p2mod_NTurbidity <- GAM_predict_p2mod(NTurbidity, mod_NTurbidity)
intercept_NTurbidity = as.numeric(attr(p2mod_NTurbidity,"constant")) 
pdata_NTurbidity <- GAM_predict_pdata(NTurbidity, mod_NTurbidity)
mod.dsig_NTurbidity <- GAM_deriv(pdata_NTurbidity, mod_NTurbidity)
linearMod <- lm(mean_turbidity ~ year, data= NTurbidity)
summary(linearMod)
dfpost1997_NTurbidity <- filter(NTurbidity, year > 1997)
linearMod_1997 <- lm(mean_turbidity ~ year, data=dfpost1997_NTurbidity)
summary(linearMod_1997)
both_nonsig(NTurbidity, NTurbidity$mean_turbidity, intercept_NTurbidity, pdata_NTurbidity, mod.dsig_NTurbidity, "Average Turbidity", "Year", "Average Spring Turbidity in Northern\n Pamlico Sound ") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "NTurbidity.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##CTurbidity 
p2mod_CTurbidity <- GAM_predict_p2mod(CTurbidity, mod_CTurbidity)
intercept_CTurbidity = as.numeric(attr(p2mod_CTurbidity,"constant")) 
pdata_CTurbidity <- GAM_predict_pdata(CTurbidity, mod_CTurbidity)
mod.dsig_CTurbidity <- GAM_deriv(pdata_CTurbidity, mod_CTurbidity)
linearMod <- lm(mean_turbidity ~ year, data= CTurbidity)
summary(linearMod)
dfpost1997_CTurbidity <- filter(CTurbidity, year > 1997)
linearMod_1997 <- lm(mean_turbidity ~ year, data=dfpost1997_CTurbidity)
summary(linearMod_1997)
both_nonsig(CTurbidity, CTurbidity$mean_turbidity, intercept_CTurbidity, pdata_CTurbidity, mod.dsig_CTurbidity, "Average Turbidity", "Year", "Average Spring Turbidity in Central\n Pamlico Sound ") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "CTurbidity.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##CBTurbidity 
p2mod_CBTurbidity <- GAM_predict_p2mod(CBTurbidity, mod_CBTurbidity)
intercept_CBTurbidity = as.numeric(attr(p2mod_CBTurbidity,"constant")) 
pdata_CBTurbidity <- GAM_predict_pdata(CBTurbidity, mod_CBTurbidity)
mod.dsig_CBTurbidity <- GAM_deriv(pdata_CBTurbidity, mod_CBTurbidity)
linearMod <- lm(mean_turbidity ~ year, data= CBTurbidity)
summary(linearMod)
dfpost1997_CBTurbidity <- filter(CBTurbidity, year > 1997)
linearMod_1997 <- lm(mean_turbidity ~ year, data=dfpost1997_CBTurbidity)
summary(linearMod_1997)
both_nonsig(CBTurbidity, CBTurbidity$mean_turbidity, intercept_CBTurbidity, pdata_CBTurbidity, mod.dsig_CBTurbidity, "Average Turbidity", "Year", "Average Spring Turbidity in Core Banks") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "CBTurbidity.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

##STurbidity 
p2mod_STurbidity <- GAM_predict_p2mod(STurbidity, mod_STurbidity)
intercept_STurbidity = as.numeric(attr(p2mod_STurbidity,"constant")) 
pdata_STurbidity <- GAM_predict_pdata(STurbidity, mod_STurbidity)
mod.dsig_STurbidity <- GAM_deriv(pdata_STurbidity, mod_STurbidity)
linearMod <- lm(mean_turbidity ~ year, data= STurbidity)
summary(linearMod)
dfpost1997_STurbidity <- filter(STurbidity, year > 1997)
linearMod_1997 <- lm(mean_turbidity ~ year, data=dfpost1997_STurbidity)
summary(linearMod_1997)
both_nonsig(STurbidity, STurbidity$mean_turbidity, intercept_STurbidity, pdata_STurbidity, mod.dsig_STurbidity, "Average Turbidity", "Year", "Average Spring Turbidity in\n Southern Region") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "STurbidity.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

#VesselsNum
p2mod_VesselsNum <- GAM_predict_p2mod(VesselsNum, mod_VesselsNum)
intercept_VesselsNum = as.numeric(attr(p2mod_VesselsNum,"constant")) 
pdata_VesselsNum <- GAM_predict_pdata(VesselsNum, mod_VesselsNum)
mod.dsig_VesselsNum <- GAM_deriv(pdata_VesselsNum, mod_VesselsNum)
linearMod <- lm(Num_reg ~ year, data= VesselsNum)
summary(linearMod)
dfpost1997_VesselsNum <- filter(VesselsNum, year > 1997)
linearMod_1997 <- lm(Num_reg ~ year, data=dfpost1997_VesselsNum)
summary(linearMod_1997)
both_sig(VesselsNum, VesselsNum$Num_reg, intercept_VesselsNum, pdata_VesselsNum, mod.dsig_VesselsNum, dfpost1997_VesselsNum, dfpost1997_VesselsNum$Num_reg, "Number Registered", "Year", "Number of Recreational Vessels Registered in NC") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))+ geom_vline(xintercept = 2007, linetype= "dotdash")
ggsave(paste0("~/Desktop/", "VesselsNum.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')

#VesselsNum_cf
p2mod_VesselsNum_cf <- GAM_predict_p2mod(VesselsNum_cf, mod_VesselsNum_cf)
intercept_VesselsNum_cf = as.numeric(attr(p2mod_VesselsNum_cf,"constant")) 
pdata_VesselsNum_cf <- GAM_predict_pdata(VesselsNum_cf, mod_VesselsNum_cf)
mod.dsig_VesselsNum_cf <- GAM_deriv(pdata_VesselsNum_cf, mod_VesselsNum_cf)
linearMod <- lm(num_reg ~ year, data= VesselsNum_cf)
summary(linearMod)
dfpost1997_VesselsNum_cf <- filter(VesselsNum_cf, year > 1997)
linearMod_1997 <- lm(num_reg ~ year, data=dfpost1997_VesselsNum_cf)
summary(linearMod_1997)
both_nonsig(VesselsNum_cf, VesselsNum_cf$num_reg, intercept_VesselsNum_cf, pdata_VesselsNum_cf, mod.dsig_VesselsNum_cf, "Number Registered", "Year", "Number of Commercial Fishing Vessels Registered in NC") + theme(plot.margin = margin(1,0.75,1,0.75, "cm"))
ggsave(paste0("~/Desktop/", "VesselsNum_cf.GAMwithTrend.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')
```
