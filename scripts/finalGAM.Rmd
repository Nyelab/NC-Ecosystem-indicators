...
mod <- gam(mean ~ s(year, k=7), data = df)
...
```{r, plot GAM}
#Run this if your linear trend post 1997 is not significant 
ggplot() + 
...
ggsave(paste0("~/NC-Ecosystem-indicators/figures/GAMS/", nospacesname, "GAMwithTrendExample.png"), width = 6, height = 4, dpi = 300, units = "in", device='png')
```

```{r, plot GAM}
##Run this if your linear trend post 1997 is significant 
ggplot() + 
  geom_line(data = df, aes(x = year, y = mean), color = 'grey53', alpha = 0.6) +
  geom_point(data = df, aes(x = year, y = mean), color = 'gray53', alpha = 0.6) + 
  geom_line(data=pdata, aes(x = year, y = p2_mod+intercept), se = FALSE, color = 'black', linetype = 'twodash', size = 1) + 
  geom_line(data = pdata, aes(y = unlist(mod.dsig$incr)+intercept, x = year), color = "blue", size = 1) + 
  geom_line(data = pdata, aes(y = unlist(mod.dsig$decr)+intercept, x = year), color = 'red', size = 1) +   geom_smooth(data = dfpost1997, aes(x = year, y = mean), method = lm, se = FALSE, color = 'darkgreen') +
  theme_bw() +
  labs (y = "UNITS HERE", x = 'Year', title = paste0(name)) + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))

#save figure. Note the output is in the figures folder.
ggsave(paste0("~/NC-Ecosystem-indicators/figures/", nospacesname, "GAMwithTrend.png"))
```